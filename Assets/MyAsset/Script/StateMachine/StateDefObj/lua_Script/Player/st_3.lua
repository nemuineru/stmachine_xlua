...
-- ステート変更のファンクション
    function QueuedStateID_J_Knife(in_entity)
        ...
        if( CurrentTime > 5 and math.abs(CurrentTime - 7) < 3  and not in_entity.attrs.isStateHit) then
            table.insert( verd, 3 ) 
        end
        if ( selfOnGrd ) then 
            table.insert( verd, 1 )
...
-- ステート変更のファンクション
    function QueuedStateID_J(in_entity)
        ...
        if( CurrentTime > 4 and math.abs(CurrentTime - 5) < 3  and not in_entity.attrs.isStateHit) then
            table.insert( verd, 3 ) 
        end
        if ( selfOnGrd ) then 
            table.insert( verd, 1 )
